...
class Engine:
    ...
    def train_models(self):
        ...
        for tf in TIMEFRAMES:
            try:
                ...
                self.model_training_ok[tf]=False; self.ai.errors[tf]=f'{type(exc).__name__}: {exc}'; traceback.print_exc()
        self.last_model=time.time() if successes else 0; return successes
    ...
    def scan(self, tf):
        ...
        try:
            ...
            setup['id']=sid
            if not self.send(self.signal_text(setup)):
                self.db.cancel(sid,'NOT_SENT'); diag['reject']='telegram_send_failed'; self.last_diagnostics[tf]=diag; return None
            diag['accepted']=True; diag['telegram_sent']=True; self.last_diagnostics[tf]=diag; return setup
        except Exception as exc:
    ...
    def diagnostics_text(self):
        ...
        for tf in TIMEFRAMES:
            ...
            lines.append(f'⏱️ <b>{tf}</b>'); lines.append(f'🤖 النموذج: {"جاهز" if d.get("model_ready") else "غير جاهز"}')
        ...
        while not self.stop_event.is_set():
            for s in self.db.all_open():
                try:
                    ...
                    if s['status']=='WAITING' and low<=s['entry']<=high:
