...
class GoldBacktest:
    ...
    def _enrich(self, tf, x):
        ...
        for htf, prefix in [('15m','ctx15'),('1h','ctx1h')]:
            ...
            if not needed:
                x[prefix+'_trend']=0; x[prefix+'_rsi']=50; x[prefix+'_macd_hist']=0; x[prefix+'_dist_ema20']=0; continue
            try:
    ...
    def _context(self, tf, row_time, x):
        ...
        if tf=='5m':
            a,b=int(r.get('ctx15_trend',0)),int(r.get('ctx1h_trend',0)); return {'trend':1 if a>0 and b>0 else (-1 if a<0 and b<0 else 0)}
        if tf=='15m': return {'trend':int(r.get('ctx1h_trend',0))}
        return {'trend':0}

    def _resolve(self, df, start, setup):
        """Require Entry touch first; then resolve SL/TP. Same-candle SL wins."""
        d=setup['direction']; entry=float(setup['entry']); sl=float(setup['sl']); tp=float(setup['tp'])
        active=False
        for j in range(start,len(df)):
            ...
            if not active:
                ...
                active=True
            if d=='BUY': hit_sl,hit_tp=lo<=sl,hi>=tp
            else: hit_sl,hit_tp=hi>=sl,lo<=tp
    ...
    @staticmethod
    def _score_band(v):
        v=float(v); return '65-69' if v<70 else ('70-74' if v<75 else ('75-79' if v<80 else '80+'))
    ...
    def _groups(trades,key):
        ...
        for k,a in groups.items():
            n=len(a); w=sum(t['result']=='TP' for t in a); net=sum(t['r'] for t in a); gw=sum(max(t['r'],0) for t in a); gl=abs(sum(min(t['r'],0) for t in a))
            out.append({'key':k,'n':n,'wr':100*w/n,'net':net,'pf':gw/gl if gl else (float('inf') if gw else 0),'e':net/n})
        return out
    ...
    def run(self, tf, bars=None, retrain_every=500):
        started=time.time(); raw=fetch(GOLD_DATA_SYMBOL,tf)
        if bars and len(raw)>bars: raw=raw.iloc[-bars:].copy()
        raw=self._completed(raw); x=self._enrich(tf,add_features(raw).dropna())
        ...
        while i<len(x)-1:
            if last_fit<0 or i-last_fit>=step:
                ...
                last_fit=i
            row=x.iloc[i]; probs={'BUY':model.probability(tf,row,'BUY'),'SELL':model.probability(tf,row,'SELL')}
            ...
            i+=1
        n=len(trades); w=sum(t['result']=='TP' for t in trades); l=n-w; net=sum(t['r'] for t in trades); gw=sum(max(t['r'],0) for t in trades); gl=abs(sum(min(t['r'],0) for t in trades))
        res={'timeframe':tf,'bars':len(x),'trades':n,'wins':w,'losses':l,'win_rate':100*w/n if n else 0,'net_r':net,'avg_score':sum(t['score'] for t in trades)/n if n else 0,'profit_factor':gw/gl if gl else (float('inf') if gw else 0),'max_drawdown_r':self._dd(trades),'expectancy_r':net/n if n else 0,'direction':self._groups(trades,'direction'),'score_bands':self._groups(trades,'score_band'),'ai_bands':self._groups(trades,'ai_band'),'elapsed_s':round(time.time()-started,1),'trades_detail':trades[-100:]}
    ...
    def format_ar(results):
        lines=['🧪 <b>Backtest حقيقي — Walk-Forward</b>','','اختبار زمني بدون استخدام شموع المستقبل في تدريب النموذج.','الدخول لا يُحسب صفقة إلا بعد لمس Entry فعليًا.','عند لمس SL وTP داخل نفس الشمعة تُحسب SL بشكل محافظ.','']
        total=wins=0; net=0.0
        for tf in TIMEFRAMES:
            r=results.get(tf,{})
            if r.get('error'): lines += [f'⏱️ <b>{tf}</b>: ❌ {r["error"]}','']; continue
            n,w,l=r.get('trades',0),r.get('wins',0),r.get('losses',0); total+=n; wins+=w; net+=r.get('net_r',0)
            pf=r.get('profit_factor',0); pf='∞' if np.isinf(pf) else f'{pf:.2f}'
