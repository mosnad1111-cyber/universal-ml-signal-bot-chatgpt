...
def _score_direction(r, direction, ai_prob, ctx):
    ...
    aligned = bullish_stack if direction == 'BUY' else bearish_stack
    partially_aligned = (ema20 > ema50) if direction == 'BUY' else (ema20 < ema50)
    structure = 1.0 if aligned else (0.62 if partially_aligned else 0.18)
    trend_span = max(abs(ema50 - ema200), atr * 0.25)
    trend_strength = _clip01(abs(ema20 - ema50) / trend_span)
    trend_score = 20.0 * (0.70 * structure + 0.30 * trend_strength)

    macd_scale = max(atr * 0.02, abs(macd_hist), 1e-12)
    if direction == 'BUY':
        rsi_strength = _clip01((rsi - 48.0) / 17.0)
        macd_strength = _clip01(0.5 + 0.5 * macd_hist / macd_scale)
    else:
        rsi_strength = _clip01((52.0 - rsi) / 17.0)
        macd_strength = _clip01(0.5 - 0.5 * macd_hist / macd_scale)
    momentum_score = 14.0 * (0.60 * rsi_strength + 0.40 * macd_strength)

    hi20 = float(r['high20']); lo20 = float(r['low20']); span20 = max(hi20 - lo20, atr)
    if direction == 'BUY':
        proximity = _clip01((close - lo20) / span20)
        breakout = 1.0 if bool(r['breakout_up']) else proximity * 0.80
    else:
        proximity = _clip01((hi20 - close) / span20)
        breakout = 1.0 if bool(r['breakout_dn']) else proximity * 0.80
    ...
    volume_strength = 0.70 if pd.isna(vol_ratio) else _clip01(float(vol_ratio) / 1.20)
...
def build_setup(df, probs, rr=2.0, divisor=3.6, sl_atr_mult=1.15,
    ...
    diagnostics['best_direction'] = direction; diagnostics['best_score'] = round(score, 1)
    ...
    if risk < 0.55 * atr or risk > 3.0 * atr:
